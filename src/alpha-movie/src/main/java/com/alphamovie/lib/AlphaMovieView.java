...
public class AlphaMovieView extends GLTextureView {
    ...
    public AlphaMovieView(Context context, int alphaColor, float accuracy) {
        super(context);

        init(alphaColor, accuracy);
    }
    ...
    private void init(int alphaColor, float accuracy) {
        setEGLContextClientVersion(GL_CONTEXT_VERSION);
        setEGLConfigChooser(8, 8, 8, 8, 16, 0);
        ...
        obtainRendererOptions(alphaColor, accuracy);
    ...
    private void obtainRendererOptions(int alphaColor, float accuracy) {
        if (alphaColor != NOT_DEFINED_COLOR) {
            renderer.setAlphaColor(alphaColor);
        }
        if (accuracy != NOT_DEFINED) {
